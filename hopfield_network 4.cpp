...
int update(int randn, int input[], double thres[], double weight[NUM][NUM], int t);
...
int hop(int input[], double thres[], double weight[NUM][NUM]){
    ...
    for(int k=0;k<TIMES;++k){
        r=rand()%NUM; rv=update(r,temp,thres,weight,k);
        cout << "\nE= " << E(temp,thres,weight) << endl;
    }
...
int update(int randn, int input[], double thres[], double weight[NUM][NUM], int t){
    ...
    //updated_v=(updated_v-thres[randn])+input[randn];
    updated_v=(updated_v-thres[randn])+alpha(input[randn],t)*input[randn];
...
double alpha(int input, int t){
    return 1-exp(-(pow((0.5-input),2.0)/VAREPSILON)*(t/LAMBDA));
}
