...
int main(){
    ...
    while(1){
        ...
        ++times_loop;
        if(times_loop==frac(NUM)/(frac(num_of_ones)*frac(NUM-num_of_ones))){
            ++num_of_ones;
            times_loop=0;
            if(num_of_ones>NUM) break;
            for(int i=0;i<num_of_ones;++i) input[i]=1;
        }
        else next_permutation(input,input+NUM);
    }
    best_ans();
    return 0;
}
...
int hop(int input[], double thres[], double weight[NUM][NUM]){
    ...
    cout << "\nLowest E= " << tempE << endl;
    if(tempE<lowest_energy){
        lowest_energy=tempE;
        for(int i=0;i<NUM;++i) best_inputs[i]=temp[i];
    }
}
...
int frac(int a){
    int sum=1;
    if(a>0) for(int i=1;i<=a;++i) sum*=i;
    return sum;
}

void best_ans(){
    cout << endl  << endl << "conclusion\n";
    cout << "lowest energy: " << lowest_energy << endl;
    cout << "the inputs at that moment:\n";
    for(int i=0;i<NUM;++i) cout << best_inputs[i] << " ";
    cout << endl;
}
